export function removeRegions(sourceCode: string) {
  ...
  let startRegionsRegex = new RegExp(
    `^//${spaceRegex}*${regionRegex}${spaceRegex}+${anythingRegex}+$`,
    "i"
  );
  let endRegionsRegex = new RegExp(
  ...
  for (let i = 0; i < lines.length; i++) {
    if (
      !startRegionsRegex.test(lines[i].trim()) &&
...
export function formatLines(sourceCode: string) {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    if (openingBraceRegex.test(lines[i]) && newLineRegex.test(lines[i + 1])) {
      // remove empty line after {
      lines.splice(i + 1, 1);

      i--;
    } else if (
      newLineRegex.test(lines[i]) &&
      ...
      i--;
    } else if (newLineRegex.test(lines[i]) && newLineRegex.test(lines[i + 1])) {
      lines.splice(i, 1);
